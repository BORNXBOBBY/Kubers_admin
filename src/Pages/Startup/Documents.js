          ...
          <div class="table-responsive">
            <table class="table">
              ...
              {startupDoc.map((item, id) => (
                <tbody>
                  <tr align="center">
                    <td> {item.document_type === 'pitch_desk' ? 'Pitch Desk': item.document_type === 'startupdocuments' ? 'Startup Documents': item.document_type}</td>
                    <td>{addSubStr(item.description, 30)}</td>
                    <td>
                      {new Date(item.uploaded_date_time).toLocaleDateString()}
                    </td>
                    <td>
                      <>
                        <Button
